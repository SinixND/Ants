...
# LBL_ObjectFiles
BUILD_DIR 				= $(OBJ_DIR_ROOT)/$(PLATFORM)/$(BUILD)
...
.PHONY: all analyze bear build bd br bt bwd bwr clean dtb init publish run rd rr rt web windows 
...
analyze:
	@cppcheck \
		...
		--error-exitcode=1 \
		--cppcheck-build-dir=$(OBJ_DIR_ROOT)/cppcheck \
		--template=gcc \
		-I include/ \
		-I src/ \
		src/
...
clean:
	...
	@rm -rf $(OBJ_DIR_ROOT)/unix/debug/*
	@rm -rf $(OBJ_DIR_ROOT)/web/debug/*
	@rm -rf $(OBJ_DIR_ROOT)/windows/debug/*
	@rm -rf $(OBJ_DIR_ROOT)/unix/release/*
	@rm -rf $(OBJ_DIR_ROOT)/web/release/*
	@rm -rf $(OBJ_DIR_ROOT)/windows/release/*
	@rm -rf $(OBJ_DIR_ROOT)/cppcheck/*
...
init:
	...
	@mkdir -p $(OBJ_DIR_ROOT)/unix/debug/
	@mkdir -p $(OBJ_DIR_ROOT)/web/debug/
	@mkdir -p $(OBJ_DIR_ROOT)/windows/debug/
	@mkdir -p $(OBJ_DIR_ROOT)/unix/release/
	@mkdir -p $(OBJ_DIR_ROOT)/web/release/
	@mkdir -p $(OBJ_DIR_ROOT)/windows/release/
	@mkdir -p $(OBJ_DIR_ROOT)/cppcheck/
...
$(OBJ_DIR_ROOT)/unix/debug/%$(OBJ_EXT) : %$(SRC_EXT) 
	$(info )
	$(info === Compile: PLATFORM=$(PLATFORM), BUILD=$(BUILD) ===)
